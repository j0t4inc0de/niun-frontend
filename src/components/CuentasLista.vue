    ...
    <div class="w-full space-y-6 relative pb-24 md:pb-0">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-mako-400">Tus credenciales seguras</p>
            </div>
            ...
            <div class="flex flex-col md:flex-row gap-3">
                <div class="relative group w-full md:w-64">
                    ...
                    <input v-model="busqueda" type="text" placeholder="Buscar..."
                        class="block w-full rounded-xl border border-white/10 bg-black/20 pl-10 pr-4 py-3 md:py-2.5 text-sm text-white placeholder-mako-500 focus:border-primary/50 focus:bg-white/5 focus:ring-0 transition-all outline-none backdrop-blur-sm">
                </div>
        ...
        <div v-else-if="cuentasFiltradas.length === 0"
            ...
            <div class="w-16 h-16 mx-auto rounded-full bg-mako-800/50 flex items-center justify-center mb-4">
                <span class="material-symbols-outlined text-mako-500 text-2xl">lock_open</span>
            </div>
            <p class="text-mako-300 font-medium">No hay cuentas aquí</p>
                ...
                <div class="relative z-10 p-4 flex items-center gap-4">
                    <div class="shrink-0" @click="abrirModalEditar(cuenta)">
                        <img v-if="cuenta.site_icon_url" :src="cuenta.site_icon_url"
                            class="w-12 h-12 rounded-xl object-cover bg-white/5 border border-white/10"
                    ...
                    <div class="flex items-center gap-2">
                        <button @click.stop="copiarTexto(cuenta.decrypted_password, cuenta.id)"
                            class="p-3 rounded-xl transition-all duration-200 bg-white/5 text-mako-300 hover:bg-white/10 hover:text-white active:scale-90"
                            :class="{ 'text-green-400 bg-green-500/10': idCopiado === cuenta.id }">
                            <span class="material-symbols-outlined text-[1.25rem]">{{ idCopiado === cuenta.id ? 'check'
                                : 'content_copy' }}</span>
                        </button>

                        <button @click.stop="irAlSitio(cuenta.site_url)"
                            class="p-3 rounded-xl bg-white/5 text-mako-300 hover:bg-primary hover:text-white transition-all duration-200 active:scale-90">
        ...
        <div v-if="modalAbierto" class="fixed inset-0 z-50 flex items-end md:items-center justify-center sm:p-4">
            <div @click="modalAbierto = false" class="absolute inset-0 bg-black/80 backdrop-blur-sm transition-opacity">
            </div>

            <div
                class="relative w-full max-w-md bg-mako-950 md:bg-mako-900 md:border md:border-white/10 rounded-t-[2rem] md:rounded-2xl shadow-2xl overflow-hidden animate-in slide-in-from-bottom-10 md:fade-in md:zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto">

                <div class="p-6 pb-2 md:border-b md:border-white/5 flex justify-between items-center bg-transparent">
                    <h3 class="text-xl font-bold text-white">
                        {{ modoEdicion ? 'Editar Cuenta' : 'Nueva Cuenta' }}
                    </h3>
                    <button @click="modalAbierto = false"
                        class="p-2 bg-white/5 rounded-full text-mako-400 hover:text-white transition-colors">
                ...
                <form @submit.prevent="guardarCuenta" class="p-6 space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div class="space-y-1.5 col-span-2 md:col-span-1">
                            <label class="text-[0.65rem] font-bold text-mako-400 uppercase tracking-wider pl-1">Sitio /
                                App</label>
                            <input v-model="form.site_name" required type="text" placeholder="Ej: Netflix"
                                class="w-full rounded-xl bg-black/20 border border-white/10 px-4 py-3.5 text-white placeholder-mako-600 focus:border-primary focus:outline-none transition-colors">
                        </div>
                        <div class="space-y-1.5 col-span-2 md:col-span-1">
                            <label
                                class="text-[0.65rem] font-bold text-mako-400 uppercase tracking-wider pl-1">URL</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[0.65rem] font-bold text-mako-400 uppercase tracking-wider pl-1">Usuario /
                            Email</label>
                        <div class="relative flex items-center">
                            <input v-model="form.email" required type="text" placeholder="usuario@email.com"
                                class="w-full rounded-xl bg-black/20 border border-white/10 px-4 py-3.5 pr-12 text-white placeholder-mako-600 focus:border-primary focus:outline-none transition-colors">
                            <button type="button" @click="copiarTexto(form.email)"
                                class="absolute right-3 p-1.5 text-mako-500 hover:text-white transition-colors">
                    ...
                    <div class="space-y-1.5">
                        <label
                            class="text-[0.65rem] font-bold text-mako-400 uppercase tracking-wider pl-1">Contraseña</label>
                        <div class="relative flex items-center">
                            <input v-model="form.password" required :type="showPassword ? 'text' : 'password'"
                                placeholder="Contraseña segura"
                                class="w-full rounded-xl bg-black/20 border border-white/10 px-4 py-3.5 pr-24 text-white placeholder-mako-600 focus:border-primary focus:outline-none transition-colors font-mono">

                            <div class="absolute right-2 flex items-center gap-1 bg-mako-900/80 rounded-lg p-0.5">
                                <button type="button" @click="showPassword = !showPassword"
                                    class="p-2 text-mako-400 hover:text-white transition-colors">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2">
                            <label
                                class="text-[0.65rem] font-bold text-yellow-500/80 uppercase tracking-wider pl-1">Llave
                                / Secreto (2FA)</label>
                            <span class="text-[0.65rem] text-mako-500 bg-white/5 px-1.5 rounded">Opcional</span>
                        </div>
                        <div class="relative flex items-center">
                            <input v-model="form.secret" :type="showSecret ? 'text' : 'password'"
                                placeholder="XXXX-XXXX-XXXX"
                                class="w-full rounded-xl bg-black/20 border border-yellow-500/20 focus:border-yellow-500/50 px-4 py-3.5 pr-24 text-white placeholder-mako-600 focus:outline-none transition-colors font-mono">
                    ...
                    <div class="pt-4 flex gap-3">
                        <button type="button" v-if="modoEdicion"
                            @click="eliminarCuenta(idEnEdicion); modalAbierto = false"
                            class="px-4 py-3.5 rounded-xl border border-red-500/20 text-red-400 hover:bg-red-500/10 transition-colors">
